...
DEFINE_string(current_mode_db_key, "/apollo/hmi/status:current_mode", "Key to store hmi_status.current_mode in KV DB.");
...
std::string TitleCase(const std::string& origin) {
        ...
        std::vector<std::string> parts = apollo::common::util::StringTokenizer::Split(origin, kDelimiter);
        for (auto& part : parts) {
                if (!part.empty()) {
...
//@zyk:将dir指定路径下的所有配置文件放到一个map里，键为文件名，值为文件路径
Map<std::string, std::string> ListFilesAsDict(const std::string& dir, const std::string& extension) {
        Map<std::string, std::string> result;
        ...
        for (const std::string& file_path : apollo::common::util::Glob(pattern)) {
                ...
                const std::string file_title = TitleCase(filename.substr(0, filename.length() - extension.length()));
...
HMIWorker::HMIWorker(const std::shared_ptr<Node>& node) : config_(LoadConfig()), node_(node) { InitStatus(); }
...
HMIConfig HMIWorker::LoadConfig() {
        ...
        //@zyk:FLAGS_hmi_modes_config_path="/apollo/modules/dreamview/conf/hmi_modes"
...
void HMIWorker::InitReadersAndWriters() {
        ...
        audio_capture_writer_ = node_->CreateWriter<AudioCapture>(FLAGS_audio_capture_topic);
        ...
        chassis_reader_ =
                node_->CreateReader<Chassis>(FLAGS_chassis_topic, [this](const std::shared_ptr<Chassis>& chassis) {
...
void HMIWorker::ChangeMode(const std::string& mode_name) {
...

        {
                ...
                HMIConfig config_
                */
                //@zyk:从某个配置文件读取HMIMode,主要是为各个module配置相关条目（最重要的就是配置启动命令）
...
void HMIWorker::StartModule(const std::string& module) const {
        //@zyk:current_mode_保存着当前hmi mode对应配置文件包含的所有配置项 module="planning"
        //@zyk:Module是从hmi_mode.proto导入的类
        /*
        message Module {
        ...
        */
        const Module* module_conf = FindOrNull(current_mode_.modules(), module);   //@zyk：找到当前的模块的配置项
        if (module_conf != nullptr) {
                //@zyk:LoadMode(current_mode_)配置的Planning模块的启动命令为：
                //@zyk:nohup mainboard -p compute_sched -d /apollo/modules/planning/dag/planning.dag &
                //@zyk:显然mainboard是程序的主入口，main函数在这里面
