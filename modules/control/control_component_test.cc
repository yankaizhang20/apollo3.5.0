...
bool ControlComponentTest::FeedTestData() {
        ...
        if (!FLAGS_test_pad_file.empty()) {
                if (!common::util::GetProtoFromFile(FLAGS_test_data_dir + FLAGS_test_pad_file, &pad_message_)) {
                        AERROR << "Failed to load PadMesssage from file " << FLAGS_test_data_dir << FLAGS_test_pad_file;
                        return false;
                }
        }
        ...
        if (!FLAGS_test_localization_file.empty()) {
                if (!common::util::GetProtoFromFile(FLAGS_test_data_dir + FLAGS_test_localization_file,
                                                    &localization_)) {
        ...
        if (!FLAGS_test_planning_file.empty()) {
                if (!common::util::GetProtoFromFile(FLAGS_test_data_dir + FLAGS_test_planning_file, &trajectory_)) {
        ...
        if (!FLAGS_test_chassis_file.empty()) {
                if (!common::util::GetProtoFromFile(FLAGS_test_data_dir + FLAGS_test_chassis_file, &chassis_)) {
        ...
        if (!FLAGS_test_monitor_file.empty()) {
                if (!apollo::common::util::GetProtoFromFile(FLAGS_test_data_dir + FLAGS_test_monitor_file,
                                                            &monitor_message_)) {
...
bool ControlComponentTest::RunControl(const std::string& test_case_name) {
        ...
        const std::string golden_result_file = apollo::common::util::StrCat("result_", test_case_name, ".pb.txt");
