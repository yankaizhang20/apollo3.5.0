...
Status LonController::ComputeControlCommand(const localization::LocalizationEstimate *localization,
        ...
        if (trajectory_analyzer_ == nullptr || //轨迹分析器
        ...
        double brake_cmd = 0.0;    //刹车
        double throttle_cmd = 0.0; //油门
        double ts = lon_controller_conf.ts();
        double preview_time = lon_controller_conf.preview_window() * ts; //预瞄时间

        if (preview_time < 0.0) {
                const auto error_msg = common::util::StrCat("Preview time set as: ", preview_time, " less than 0");
                AERROR << error_msg;
        ...
        } else if (VehicleStateProvider::Instance()->linear_velocity() <= lon_controller_conf.switch_speed()) {
        ...
        speed_controller_input_limited = common::math::Clamp(speed_controller_input, -speed_controller_input_limit,
        ...
        double slope_offset_compenstaion =
                digital_filter_pitch_angle_.Filter(GRA_ACC * std::sin(VehicleStateProvider::Instance()->pitch()));
        ...
        if (FLAGS_use_preview_speed_for_table) {   //false
                ...
                calibration_value =
                        control_interpolation_->Interpolate(std::make_pair(chassis_->speed_mps(), acceleration_lookup));
        }

        if (calibration_value >= 0) {
                throttle_cmd = std::abs(calibration_value) > throttle_deadzone ? std::abs(calibration_value)
                                                                               : throttle_deadzone;
                brake_cmd = 0.0;
        } else {
                throttle_cmd = 0.0;
                brake_cmd = std::abs(calibration_value) > brake_deadzone ? std::abs(calibration_value) : brake_deadzone;
        }
        ...
        if (FLAGS_enable_csv_debug && speed_log_file_ != nullptr) {
                fprintf(speed_log_file_,
                        ...
                        debug->acceleration_lookup(), debug->speed_lookup(), calibration_value, throttle_cmd, brake_cmd,
                        debug->is_full_stop());
        }
...
void LonController::ComputeLongitudinalErrors(const TrajectoryAnalyzer *trajectory_analyzer, const double preview_time,
        ...
        auto matched_point = trajectory_analyzer->QueryMatchedPathPoint(VehicleStateProvider::Instance()->x(),
                                                                        VehicleStateProvider::Instance()->y());

        trajectory_analyzer->ToTrajectoryFrame(
                VehicleStateProvider::Instance()->x(), VehicleStateProvider::Instance()->y(),
                VehicleStateProvider::Instance()->heading(), VehicleStateProvider::Instance()->linear_velocity(),
