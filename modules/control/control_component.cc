...
bool ControlComponent::Init() {
        ...
        CHECK(apollo::common::util::GetProtoFromFile(FLAGS_control_conf_file, &control_conf_))
...
Status ControlComponent::ProduceControlCommand(ControlCommand *control_command) {
        ...
        // check estop
        estop_ = control_conf_.enable_persistent_estop() ? estop_ || local_view_.trajectory.estop().is_estop()
...
bool ControlComponent::Proc() {
        ...
        control_command.mutable_latency_stats()->set_total_time_exceeded(time_diff_ms < control_conf_.control_period());
...
Status ControlComponent::CheckInput(LocalView *local_view) {
        ...
        if (!local_view->trajectory.estop().is_estop() && local_view->trajectory.trajectory_point_size() == 0) {
                AWARN_EVERY(100) << "planning has no trajectory point. ";
        ...
        for (auto &trajectory_point : *local_view->trajectory.mutable_trajectory_point()) {
        ...
        VehicleStateProvider::Instance()->Update(local_view->localization, local_view->chassis);
...
Status ControlComponent::CheckTimestamp(const LocalView &local_view) {
        ...
        if (localization_diff > (control_conf_.max_localization_miss_num() * control_conf_.localization_period())) {
        ...
        if (trajectory_diff > (control_conf_.max_planning_miss_num() * control_conf_.trajectory_period())) {
