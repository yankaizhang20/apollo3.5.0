...
bool ControlTestBase::test_control() {
        ...
        if (!FLAGS_test_pad_file.empty()) {
                ...
                if (!common::util::GetProtoFromFile(FLAGS_test_data_dir + FLAGS_test_pad_file, &pad_message)) {
        ...
        if (!FLAGS_test_localization_file.empty()) {
                ...
                control_.OnLocalization(std::make_shared<apollo::localization::LocalizationEstimate>(localization));
        }

        // Planning
        if (!FLAGS_test_planning_file.empty()) {
                ADCTrajectory trajectory;
                if (!common::util::GetProtoFromFile(FLAGS_test_data_dir + FLAGS_test_planning_file, &trajectory)) {
        ...
        if (!FLAGS_test_chassis_file.empty()) {
                Chassis chassis;
                if (!common::util::GetProtoFromFile(FLAGS_test_data_dir + FLAGS_test_chassis_file, &chassis)) {
