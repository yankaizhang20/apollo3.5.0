...
void PlanningBase::SetFallbackTrajectory(ADCTrajectory* const trajectory_pb) {
        ...
        if (last_planning_ != nullptr) {
                ...
                for (int i = 0; i < last_planning_->trajectory_point_size(); ++i) {
                        const double t = last_planning_->trajectory_point(i).relative_time() + pre_time_stamp -
                                         current_time_stamp;
...
void PlanningBase::ExportChart(const planning_internal::Debug& debug_info, planning_internal::Debug* debug_chart) {
        ...
        AddSpeedPlan(debug_info.planning_data().speed_plan(), debug_chart->mutable_planning_data()->add_chart());
}
