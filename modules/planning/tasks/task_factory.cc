...
void TaskFactory::Init(const PlanningConfig& config) {
        task_factory_.Register(TaskConfig::DP_ST_SPEED_OPTIMIZER,
                               [](const TaskConfig& config) -> Task* { return new DpStSpeedOptimizer(config); });    //@zyk:lambada表达式
        task_factory_.Register(TaskConfig::QP_SPLINE_PATH_OPTIMIZER,
                               [](const TaskConfig& config) -> Task* { return new QpSplinePathOptimizer(config); });
        task_factory_.Register(TaskConfig::SIDE_PASS_PATH_DECIDER,
                               [](const TaskConfig& config) -> Task* { return new SidePassPathDecider(config); });
        task_factory_.Register(TaskConfig::DP_POLY_PATH_OPTIMIZER,
                               [](const TaskConfig& config) -> Task* { return new DpPolyPathOptimizer(config); });
        task_factory_.Register(TaskConfig::PATH_DECIDER,
                               [](const TaskConfig& config) -> Task* { return new PathDecider(config); });
        task_factory_.Register(TaskConfig::SPEED_DECIDER,
                               [](const TaskConfig& config) -> Task* { return new SpeedDecider(config); });
        task_factory_.Register(TaskConfig::QP_SPLINE_ST_SPEED_OPTIMIZER,
                               [](const TaskConfig& config) -> Task* { return new QpSplineStSpeedOptimizer(config); });
        task_factory_.Register(TaskConfig::QP_PIECEWISE_JERK_PATH_OPTIMIZER, [](const TaskConfig& config) -> Task* {
                return new QpPiecewiseJerkPathOptimizer(config);
        });
        task_factory_.Register(TaskConfig::PROCEED_WITH_CAUTION_SPEED, [](const TaskConfig& config) -> Task* {
                return new ProceedWithCautionSpeedGenerator(config);
        });
        task_factory_.Register(TaskConfig::DECIDER_CREEP,
                               [](const TaskConfig& config) -> Task* { return new DeciderCreep(config); });
        task_factory_.Register(TaskConfig::DECIDER_RULE_BASED_STOP,
                               [](const TaskConfig& config) -> Task* { return new DeciderRuleBasedStop(config); });
        task_factory_.Register(TaskConfig::SIDE_PASS_SAFETY,
                               [](const TaskConfig& config) -> Task* { return new SidePassSafety(config); });
        ...
        for (const auto& default_task_config : config.default_task_config()) {
                default_task_configs[default_task_config.task_type()] = default_task_config;
        }
}
