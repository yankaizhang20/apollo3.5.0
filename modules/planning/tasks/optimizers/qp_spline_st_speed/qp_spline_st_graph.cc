...
QpSplineStGraph::QpSplineStGraph(Spline1dSolver* spline_solver, const QpStSpeedConfig& qp_st_speed_config,
                                 const VehicleParam& veh_param, const bool is_change_lane)
...
void QpSplineStGraph::Init() {
        ...
        uint32_t num_evaluated_t = 10 * num_spline + 1; //采样点
...
Status QpSplineStGraph::Search(const StGraphData& st_graph_data, const std::pair<double, double>& accel_bound,
        ...
        //约束设置
        if (!AddConstraint(st_graph_data.init_point(), st_graph_data.speed_limit(), st_graph_data.st_boundaries(),
                           accel_bound)
                     .ok()) {
        ...
        ADEBUG << "QpSplineStGraph time to solve is " << (end - start) * 1000 << " ms.";
...
Status QpSplineStGraph::AddConstraint(const common::TrajectoryPoint& init_point, const SpeedLimit& speed_limit,
        ...
        if (!constraint->AddPointConstraint(0.0, 0.0)) { //要经过原点
                const std::string msg = "add st start point constraint failed";
                AERROR << msg;
                return Status(ErrorCode::PLANNING_ERROR, msg);
        }

        if (!constraint->AddPointDerivativeConstraint(0.0, init_point_.v())) {//原点处的斜率要等于初始点（规划起始点）的速度
                const std::string msg = "add st start point velocity constraint failed!";
                AERROR << msg;
                return Status(ErrorCode::PLANNING_ERROR, msg);
        }

        // monotone constraint  单调增约束
        ...
        // smoothness constraint 函数连接平滑约束
        ...
        for (const double curr_t : t_evaluated_) {
                double lower_s = 0.0;
                double upper_s = 0.0;
                GetSConstraintByTime(boundaries, curr_t, qp_st_speed_config_.total_path_length(), &upper_s, &lower_s);
                s_upper_bound.push_back(upper_s);
                s_lower_bound.push_back(lower_s);
                ADEBUG << "Add constraint by time: " << curr_t << " upper_s: " << upper_s << " lower_s: " << lower_s;
        }
        ...
        // acceleration constraint  加速度约束 配置文件中未出了加速度的最大最小值
        ...
        if (!constraint->AddSecondDerivativeBoundary(t_evaluated_, accel_lower_bound, accel_upper_bound)) {
...
Status QpSplineStGraph::AddKernel(const std::vector<const StBoundary*>& boundaries, const SpeedLimit& speed_limit) {
        ...
        //跟车的时候，距离一定要保持在一定范围内，不能被甩的太开，也就是跟进前车，保持效率
        if (!AddFollowReferenceLineKernel(boundaries, qp_st_speed_config_.qp_spline_config().follow_weight()).ok()) {
                return Status(ErrorCode::PLANNING_ERROR, "QpSplineStGraph::AddKernel");
        }

        //缓行和上面一样
        if (!AddYieldReferenceLineKernel(boundaries, qp_st_speed_config_.qp_spline_config().yield_weight()).ok()) {
                return Status(ErrorCode::PLANNING_ERROR, "QpSplineStGraph::AddKernel");
        }

        //qp和dp规划的st相差不能太多，但是这里权重为0，也就是这一项不起作用
...
Status QpSplineStGraph::Solve() {
        return spline_solver_->Solve() ? Status::OK() : Status(ErrorCode::PLANNING_ERROR, "QpSplineStGraph::solve");
}
...
Status QpSplineStGraph::AddCruiseReferenceLineKernel(const double weight) {
        ...
        if (t_evaluated_.size() > 0) {
                spline_kernel->AddReferenceLineKernelMatrix(
                        ...
                        weight * qp_st_speed_config_.total_time() / static_cast<double>(t_evaluated_.size()));
        }

        return Status::OK();
}

Status QpSplineStGraph::AddFollowReferenceLineKernel(const std::vector<const StBoundary*>& boundaries,
                                                     const double weight) {
        auto* spline_kernel = spline_solver_->mutable_spline_kernel();
        ...
        for (size_t i = 0; i < filtered_evaluate_t.size(); ++i) {
                ADEBUG << "Follow Ref S: " << ref_s[i] << " Relative time: " << filtered_evaluate_t[i] << std::endl;
        }
        return Status::OK();
}

Status QpSplineStGraph::AddYieldReferenceLineKernel(const std::vector<const StBoundary*>& boundaries,
                                                    const double weight) {
        auto* spline_kernel = spline_solver_->mutable_spline_kernel();
        std::vector<double> ref_s;
        std::vector<double> filtered_evaluate_t;
        ...
        for (size_t i = 0; i < filtered_evaluate_t.size(); ++i) {
                ADEBUG << "Yield Ref S: " << ref_s[i] << " Relative time: " << filtered_evaluate_t[i] << std::endl;
        }
        return Status::OK();
}
...
bool QpSplineStGraph::AddDpStReferenceKernel(const double weight) const {
        ...
        if (!t_pos.empty()) {
                spline_kernel->AddReferenceLineKernelMatrix(
                        t_pos, s_pos, weight * qp_st_speed_config_.total_time() / static_cast<double>(t_pos.size()));
        }
        return true;
}

Status QpSplineStGraph::GetSConstraintByTime(const std::vector<const StBoundary*>& boundaries, const double time,
        ...
        if (static_cast<double>(t_evaluated_.size() + speed_limit.speed_limit_points().size()) <
            static_cast<double>(t_evaluated_.size()) *
                ...
                for (const double t : t_evaluated_) {
                        ...
                        const auto& it = std::lower_bound(speed_limit_points.begin(), speed_limit_points.end(), s, cmp);
