...
TrajectoryCost::TrajectoryCost(const DpPolyPathConfig &config, const ReferenceLine &reference_line,
                               const bool is_change_lane_path, const std::vector<const Obstacle *> &obstacles,
                               const common::VehicleParam &vehicle_param, const SpeedData &heuristic_speed_data,
                               const common::SLPoint &init_sl_point, const SLBoundary &adc_sl_boundary)
    : config_(config)
    ...
    , adc_sl_boundary_(adc_sl_boundary) {
        const double total_time = std::min(heuristic_speed_data_.TotalTime(), FLAGS_prediction_total_time);

        //预测障碍物轨迹总帧数，eval_time_interval为predict模块障碍物两个预测轨迹点的时间差
...
//动态障碍物的cost，根据每个预测帧下车辆位置和障碍物位置的相对关系确定
...
ComparableCost TrajectoryCost::GetCostFromObsSL(const double adc_s, const double adc_l,
        ...
        bool no_overlap =
                ((adc_front_s < obs_sl_boundary.start_s() || adc_end_s > obs_sl_boundary.end_s()) || // longitudinal
        ...
        const double delta_l = std::fmax(adc_right_l - obs_sl_boundary.end_l(), obs_sl_boundary.start_l() - adc_left_l);
        /*
        ...
        */
        ...
        if (delta_l < kSafeDistance) {
                obstacle_cost.safety_cost +=
                        config_.obstacle_collision_cost() * Sigmoid(config_.obstacle_collision_distance() - delta_l);
        }
