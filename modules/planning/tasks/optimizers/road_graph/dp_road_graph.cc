...
DpRoadGraph::DpRoadGraph(const DpPolyPathConfig &config, const ReferenceLineInfo &reference_line_info,
...
bool DpRoadGraph::FindPathTunnel(const common::TrajectoryPoint &init_point,
                                 const std::vector<const Obstacle *> &obstacles, PathData *const path_data) {
...
bool DpRoadGraph::GenerateMinCostPath(const std::vector<const Obstacle *> &obstacles,
        ...
        TrajectoryCost trajectory_cost(config_, reference_line_, reference_line_info_.IsChangeLanePath(), obstacles,
                                       vehicle_config.vehicle_param(), speed_data_, init_sl_point_,
        ...
        for (const auto &node : *min_cost_path) {
                ...
                planning_debug_->mutable_planning_data()->mutable_dp_poly_graph()->add_min_cost_point()->CopyFrom(
                        node.sl_point);
        }
        return true;
}
...
void DpRoadGraph::UpdateNode(const std::shared_ptr<RoadGraphMessage> &msg) {
        ...
        if (reference_line_info_.IsChangeLanePath() && msg->level >= 2) {
                ...
                QuinticPolynomialCurve1d curve(init_sl_point_.l(), init_dl, init_ddl, msg->cur_node->sl_point.l(), 0.0,
