...
void WaypointSampler::Init(const ReferenceLineInfo *reference_line_info, const common::SLPoint &init_sl_point,
...
bool WaypointSampler::SamplePathWaypoints(const common::TrajectoryPoint &init_point,
        ...
        if (!status->has_pull_over() && status->pull_over().in_pull_over()) {
                ...
                if (!reference_line_info_->reference_line().XYToSL(start_point, &start_point_sl)) {
                ...
                if (init_sl_point_.s() > start_point_sl.s()) {
                        ...
                        if (!reference_line_info_->reference_line().XYToSL(stop_point, &stop_point_sl)) {
                                AERROR << "Fail to change xy to sl.";
                                return false;
                        }
        ...
        for (size_t i = 0; i < kNumLevel && accumulated_s < total_length; ++i) {
                ...
                if (!level_points.empty()) {
                        planning_debug_->mutable_planning_data()->mutable_dp_poly_graph()->add_sample_layer()->CopyFrom(
                                sample_layer_debug);
                        points->emplace_back(level_points);
                }
        }
