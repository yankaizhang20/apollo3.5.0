...
class DpRoadGraph {
    public:
        explicit DpRoadGraph(const DpPolyPathConfig &config, const ReferenceLineInfo &reference_line_info,
                             const SpeedData &speed_data);

        ~DpRoadGraph() = default;

        bool FindPathTunnel(const common::TrajectoryPoint &init_point, const std::vector<const Obstacle *> &obstacles,
                            PathData *const path_data);

        void SetDebugLogger(apollo::planning_internal::Debug *debug) { planning_debug_ = debug; }

        void SetWaypointSampler(WaypointSampler *waypoint_sampler) { waypoint_sampler_.reset(waypoint_sampler); }
    ...
    private:
        ...
        struct DpRoadGraphNode {
            public:
                ...
                void UpdateCost(const DpRoadGraphNode *node_prev, const QuinticPolynomialCurve1d &curve,
                                const ComparableCost &cost) {
                        if (cost <= min_cost) {
                                min_cost = cost;
        ...
        struct RoadGraphMessage {
                RoadGraphMessage(const std::list<DpRoadGraphNode> &_prev_nodes, const uint32_t _level,
                                 const uint32_t _total_level, TrajectoryCost *_trajectory_cost, DpRoadGraphNode *_front,
