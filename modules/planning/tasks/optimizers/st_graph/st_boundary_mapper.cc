...
StBoundaryMapper::StBoundaryMapper(const SLBoundary& adc_sl_boundary, const StBoundaryConfig& config,
                                   const ReferenceLine& reference_line, const PathData& path_data,
                                   const double planning_distance, const double planning_time, bool is_change_lane)
...
Status StBoundaryMapper::CreateStBoundary(PathDecision* path_decision) const {
        ...
        for (const auto* const_obstacle : obstacles.Items()) {
                ...
                if (!is_bycycle_or_pedestrain && obstacle->IsStatic() && lat_decision.has_nudge()) {
                        ADEBUG << "Do not map S-T Boundary for nudge decision: " << lat_decision.DebugString();
                        continue;
                }

                if (!obstacle->HasLongitudinalDecision()) {
                        if (!MapWithoutDecision(obstacle).ok()) {
                                std::string msg = StrCat("Fail to map obstacle ", obstacle->Id(), " without decision.");
                                AERROR << msg;
                                return Status(ErrorCode::PLANNING_ERROR, msg);
                        }
                        continue;
                ...
                if (decision.has_stop()) {
                        const double stop_s = obstacle->PerceptionSLBoundary().start_s() + decision.stop().distance_s();
...
Status StBoundaryMapper::CreateStBoundaryWithHistory(const ObjectDecisions& history_decisions,
        ...
        for (const auto& history_decision : history_decisions.decision()) {
                for (const auto& decision : history_decision.object_decision()) {
                        if (Obstacle::IsLongitudinalDecision(decision) && !decision.has_ignore()) {
...
bool StBoundaryMapper::MapStopDecision(Obstacle* stop_obstacle, const ObjectDecisionType& stop_decision) const {
        DCHECK(stop_decision.has_stop()) << "Must have stop decision";

        if (stop_obstacle->PerceptionSLBoundary().start_s() > adc_sl_boundary_.end_s() + planning_distance_) {
                return true;
        }

        double st_stop_s = 0.0;
        const double stop_ref_s = stop_obstacle->PerceptionSLBoundary().start_s() + stop_decision.stop().distance_s() -
        ...
        if (stop_ref_s > path_data_.frenet_frame_path().back().s()) {
                ...
                if (!path_data_.GetPathPointWithRefS(stop_ref_s, &stop_point)) {
                        AERROR << "Fail to get path point from reference s. The sl boundary of "
                                  "stop obstacle "
                               << stop_obstacle->Id() << " is: " << stop_obstacle->PerceptionSLBoundary().DebugString();
                        return false;
                }
...
Status StBoundaryMapper::MapWithoutDecision(Obstacle* obstacle) const {
        ...
        if (!GetOverlapBoundaryPoints(path_data_.discretized_path(), *obstacle, &upper_points, &lower_points)) {
                return Status::OK();
        }
        ...
        if (trajectory.trajectory_point_size() == 0) {
                ...
                for (const auto& curr_point_on_path : path_points) {
                        ...
                        if (CheckOverlap(curr_point_on_path, obs_box, st_boundary_config_.boundary_buffer())) {
                                ...
                                double high_s =
                                        std::fmin(planning_distance_, curr_point_on_path.s() + forward_distance);
...
//TODO:
Status StBoundaryMapper::MapWithDecision(Obstacle* obstacle, const ObjectDecisionType& decision) const {
        DCHECK(decision.has_follow() || decision.has_yield() || decision.has_overtake())
                << "decision is " << decision.DebugString() << ", but it must be follow or yield or overtake.";
        ...
        if (decision.has_follow()) {
                ...
                boundary = StBoundary::GenerateStBoundary(lower_points, upper_points).ExpandByS(characteristic_length);
...
bool StBoundaryMapper::CheckOverlap(const PathPoint& path_point, const Box2d& obs_box, const double buffer) const {
        ...
        Vec2d vec_to_center =
                Vec2d((vehicle_param_.front_edge_to_center() - vehicle_param_.back_edge_to_center()) / 2.0,
                      (vehicle_param_.left_edge_to_center() + left_delta_l - vehicle_param_.right_edge_to_center() +
                       right_delta_l) /
                              2.0)
