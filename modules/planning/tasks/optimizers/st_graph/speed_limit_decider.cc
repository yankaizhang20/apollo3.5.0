...
SpeedLimitDecider::SpeedLimitDecider(const SLBoundary& adc_sl_boundary, const StBoundaryConfig& config,
...
Status SpeedLimitDecider::GetSpeedLimits(const IndexedList<std::string, Obstacle>& obstacles,
        ...
        for (uint32_t i = 0; i < discretized_path.size(); ++i) {
                ...
                //a = v ^2 / R = v^2 · kappa   v = sqrt(a / kappa)
                ...
                // -- 2.2: limit by centripetal jerk  向心力限制
                double centri_jerk_speed_limit = std::numeric_limits<double>::max();
                if (i + 1 < discretized_path.size()) {
                        const double ds = discretized_path.at(i + 1).s() - discretized_path.at(i).s();
                        DCHECK_GE(ds, 0.0);
                ...
                for (const auto* const_obstacle : obstacles.Items()) {
                        ...
                        if (is_close_on_left || is_close_on_right) {
                                ...
                                if (const_obstacle->IsStatic()) {
                                        nudge_speed_ratio = st_boundary_config_.static_obs_nudge_speed_ratio();//0.6
                                } else {
                                        nudge_speed_ratio = st_boundary_config_.dynamic_obs_nudge_speed_ratio();//0.8
                                }
                ...
                if (FLAGS_enable_nudge_slowdown) {//enable_nudge_slowdown==true
...
double SpeedLimitDecider::GetCentricAccLimit(const double kappa) const {
        ...
        if (std::fabs(v_high - v_low) < 1.0) {
                AERROR << "High speed and low speed threshold are too close to each other. "
                       ...
                       << " Current high speed threshold = " << v_high << ", current low speed threshold = " << v_low;
                return h_v_acc;
        }
