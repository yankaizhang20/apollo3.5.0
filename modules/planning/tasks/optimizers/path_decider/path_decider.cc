...
PathDecider::PathDecider(const TaskConfig &config) : Task(config) { SetName("PathDecider"); }
...
bool PathDecider::MakeStaticObstacleDecision(const PathData &path_data, PathDecision *const path_decision) {
        ...
        for (const auto *obstacle : path_decision->obstacles().Items()) {
                ...
                //从这里可以看出障碍物的l是从右向左递增，障碍物的start_l为最右边
                ...
                } else if (obstacle->IsLaneBlocking()) {//障碍物阻塞车道
                        // stop
