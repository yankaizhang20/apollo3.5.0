...
SpeedDecider::SpeedDecider(const TaskConfig& config) : Task(config) { SetName("SpeedDecider"); }
...
//判断障碍物在车辆的前方、后方、交叉
SpeedDecider::StPosition SpeedDecider::GetStPosition(const PathDecision* const path_decision,
...
bool SpeedDecider::CheckKeepClearCrossable(const PathDecision* const path_decision, const SpeedData& speed_profile,
        ...
        if (last_speed_point.s() <= keep_clear_st_boundary.max_s() && last_speed_point_v < kKeepClearSlowSpeed) {
...
bool SpeedDecider::IsFollowTooClose(const Obstacle& obstacle) const {
        ...
        const double distance = obstacle.st_boundary().min_s() - FLAGS_min_stop_distance_obstacle;
                ...
                switch (position) {
                        ...
                        case CROSS:  //如果无人车和障碍物路径有重叠
                                if (mutable_obstacle->IsBlockingObstacle()) {
                                        ...
                                        const std::string msg = "Failed to find a solution for crossing obstacle:" +
                                                                mutable_obstacle->Id();
                                        AERROR << msg;
                                        return Status(ErrorCode::PLANNING_ERROR, msg);
                                }
...
bool SpeedDecider::CreateStopDecision(const Obstacle& obstacle, ObjectDecisionType* const stop_decision,
...
bool SpeedDecider::CreateFollowDecision(const Obstacle& obstacle, ObjectDecisionType* const follow_decision) const {
        ...
        const double follow_distance_s = -std::fmax(follow_speed * FLAGS_follow_time_buffer, FLAGS_follow_min_distance);
...
bool SpeedDecider::CreateOvertakeDecision(const Obstacle& obstacle, ObjectDecisionType* const overtake_decision) const {
        ...
        const double overtake_distance_s =
                std::fmax(std::fmax(init_point_.v(), obstacle_speed) * kOvertakeTimeBuffer, kMinOvertakeDistance);
...
bool SpeedDecider::CheckIsFollowByT(const StBoundary& boundary) const {
        if (boundary.BottomLeftPoint().s() > boundary.BottomRightPoint().s()) {//是迎面驶来的目标
                return false;
        }
        constexpr double kFollowTimeEpsilon = 1e-3;
        constexpr double kFollowCutOffTime = 0.5;
        if (boundary.min_t() > kFollowCutOffTime || boundary.max_t() < kFollowTimeEpsilon) {//目标是否适于跟踪
                return false;
        }
        return true;
}
