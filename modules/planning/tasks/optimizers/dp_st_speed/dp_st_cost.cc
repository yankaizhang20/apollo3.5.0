...
DpStCost::DpStCost(const DpStSpeedConfig& config, const std::vector<const Obstacle*>& obstacles,
...
double DpStCost::GetReferenceCost(const STPoint& point, const STPoint& reference_point) const {
        return config_.reference_weight() * (point.s() - reference_point.s()) * (point.s() - reference_point.s()) *
               unit_t_;
}
