...
DpStSpeedOptimizer::DpStSpeedOptimizer(const TaskConfig& config) : SpeedOptimizer(config) {
...
bool DpStSpeedOptimizer::SearchStGraph(const StBoundaryMapper& boundary_mapper,
        ...
        DpStGraph st_graph(st_graph_data, dp_st_speed_config_,
                           reference_line_info_->path_decision()->obstacles().Items(), init_point_, adc_sl_boundary_);
...
Status DpStSpeedOptimizer::Process(const SLBoundary& adc_sl_boundary, const PathData& path_data,
        ...
        //total_path_length: 149  total_time: 7.0  
        StBoundaryMapper boundary_mapper(adc_sl_boundary, st_boundary_config_, *reference_line_, path_data,
