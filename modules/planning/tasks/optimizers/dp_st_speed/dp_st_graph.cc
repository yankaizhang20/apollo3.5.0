...
bool CheckOverlapOnDpStGraph(const std::vector<const StBoundary*>& boundaries, const StGraphPoint& p1,
...
DpStGraph::DpStGraph(const StGraphData& st_graph_data, const DpStSpeedConfig& dp_config,
    ...
    , adc_sl_boundary_(adc_sl_boundary) {
        ...
        unit_s_ = dp_st_speed_config_.total_path_length() / (dp_st_speed_config_.matrix_dimension_s() - 1);
...
Status DpStGraph::Search(SpeedData* const speed_data) {
        ...
        if (st_graph_data_.st_boundaries().empty()) {
                ...
                double t = 0.0;
                for (int i = 0;
                     i <= dp_st_speed_config_.matrix_dimension_t() && i <= dp_st_speed_config_.matrix_dimension_s();
                     ++i, t += unit_t_, s += unit_s_) {
                        SpeedPoint speed_point;
                        speed_point.set_s(s);
                        speed_point.set_t(t);
                        const double v_default = unit_s_ / unit_t_; //  1m/s
        ...
        // B 循环计算cost表中每个元素的大小，也就是初始规划点init_point到(相对时刻t，累积距离s)p2点的最小开销cost
                ...
                if (count > 0) {
                        ...
                        for (size_t r = next_lowest_row; r <= next_highest_row; ++r) {
                                ...
                                if (FLAGS_enable_multi_thread_in_dp_st_graph) {
                                        results.push_back(cyber::Async(&DpStGraph::CalculateCostAt, this, msg));
                                } else {
                                        CalculateCostAt(msg);
                                }
                        }
...
void DpStGraph::GetRowRange(const StGraphPoint& point, size_t* next_highest_row, size_t* next_lowest_row) {
        double v0 = 0.0;
        if (!point.pre_point()) {
                v0 = init_point_.v();
        } else {
        ...
        const double delta_s_upper_bound = v0 * unit_t_ + vehicle_param_.max_acceleration() * speed_coeff;
...
void DpStGraph::CalculateCostAt(const std::shared_ptr<StGraphMessage>& msg) {
        ...
        if (c == 2) {//第二层 （第二个时间单位）
                for (uint32_t r_pre = r_low; r_pre <= r; ++r_pre) {
                        ...
                        const double cost = cost_cr.obstacle_cost() + pre_col[r_pre].total_cost() +
        ...
        for (uint32_t r_pre = r_low; r_pre <= r; ++r_pre) {
                ...
                const double curr_a = (cost_cr.index_s() * unit_s_ + pre_col[r_pre].pre_point()->index_s() * unit_s_ -
...
double DpStGraph::CalculateEdgeCost(const STPoint& first, const STPoint& second, const STPoint& third,
...
double DpStGraph::CalculateEdgeCostForSecondCol(const uint32_t row, const double speed_limit) {
...
double DpStGraph::CalculateEdgeCostForThirdCol(const uint32_t curr_row, const uint32_t pre_row,
