...
RssDecider::RssDecider(const TaskConfig &config) : Task(config) { SetName("RssDecider"); }
...
Status RssDecider::Process(Frame *frame, ReferenceLineInfo *reference_line_info) {
        ...
        for (const auto *obstacle : path_decision->obstacles().Items()) {
                ...
                bool is_on_road = reference_line_info->reference_line().HasOverlap(obstacle->PerceptionBoundingBox());
                if (!is_on_road) {
                        continue;
                }
        ...
        reference_line_info->reference_line().GetLaneWidth(ego_v_s_start, &left_width_ego, &right_width_ego);

        double left_width_obs = 0.0;
        double right_width_obs = 0.0;
        reference_line_info->reference_line().GetLaneWidth(nearest_obs_s_start, &left_width_obs, &right_width_obs);
        ...
        ADEBUG << "Task " << Name() << " nearest_obstacle_distance = " << front_obstacle_distance
               << " obs_s_start = " << nearest_obs_s_start << " obs_s_end = " << nearest_obs_s_end
               << " obs_l_start = " << nearest_obs_l_start << " obs_l_end = " << nearest_obs_l_end
               << " obs_speed = " << nearest_obs_speed << " ego_v_s_start = " << ego_v_s_start
               << " ego_v_s_end = " << ego_v_s_end << " ego_v_l_start = " << ego_v_l_start
               << " ego_v_l_end = " << ego_v_l_end << " lane_width_ego = " << total_lane_width_ego
               << " lane_width_obs = " << total_lane_width_obs << " left_width_obs = " << left_width_obs
        ...
        leadingObject = rss::createObject(nearest_obs_speed, 0.0); // TODO(l1212s): refine
        leadingObject.objectId = 0;
        rss::world::OccupiedRegion occupiedRegion_leading;
        occupiedRegion_leading.segmentId = 0;
        occupiedRegion_leading.lonRange.minimum = nearest_obs_s_start / total_lane_length;
        occupiedRegion_leading.lonRange.maximum = nearest_obs_s_end / total_lane_length;
        occupiedRegion_leading.latRange.minimum =
                std::max((left_width_obs - nearest_obs_l_end) / total_lane_width_obs, 0.0);
        occupiedRegion_leading.latRange.maximum =
                std::min((left_width_obs - nearest_obs_l_start) / total_lane_width_obs, 1.0);
        leadingObject.occupiedRegions.push_back(occupiedRegion_leading);

        ADEBUG << " occupiedRegion_leading.lonRange.minimum = " << occupiedRegion_leading.lonRange.minimum
               << " occupiedRegion_leading.lonRange.maximum = " << occupiedRegion_leading.lonRange.maximum
               << " occupiedRegion_leading.latRange.minimum = " << occupiedRegion_leading.latRange.minimum
               << " occupiedRegion_leading.latRange.maximum = " << occupiedRegion_leading.latRange.maximum;

        followingObject = rss::createObject(adc_velocity, 0.0); // TODO(l1212s): refine
        ...
        occupiedRegion_following.latRange.minimum = (left_width_ego - ego_v_l_end) / total_lane_width_ego;
        occupiedRegion_following.latRange.maximum = (left_width_ego - ego_v_l_start) / total_lane_width_ego;
        followingObject.occupiedRegions.push_back(occupiedRegion_following);

        ADEBUG << " adc_velocity = " << adc_velocity
               << " occupiedRegion_following.lonRange.minimum = " << occupiedRegion_following.lonRange.minimum
               << " occupiedRegion_following.lonRange.maximum = " << occupiedRegion_following.lonRange.maximum
               << " occupiedRegion_following.latRange.minimum = " << occupiedRegion_following.latRange.minimum
               << " occupiedRegion_following.latRange.maximum = " << occupiedRegion_following.latRange.maximum;
        ...
        laneSegment.width.minimum = std::min(total_lane_width_ego, total_lane_width_obs);
        laneSegment.width.maximum = std::max(total_lane_width_ego, total_lane_width_obs);
        ...
        double dMin_lon = rss::calculateLongitudinalMinSafeDistance(followingObject, leadingObject);
        rss::world::AccelerationRestriction accelerationRestriction;
        rss::core::RssCheck rssCheck;
        rssCheck.calculateAccelerationRestriction(worldModel, accelerationRestriction);
        ...
        ADEBUG << " acc_longitudianlRange_minimum: " << reference_line_info->rss_info().acc_lon_range_minimum();
        ADEBUG << " acc_longitudinalRange_maximum: " << reference_line_info->rss_info().acc_lon_range_maximum();
        ADEBUG << " acc_lateralLeftRange_minimum: " << reference_line_info->rss_info().acc_lat_left_range_minimum();
        ADEBUG << " acc_lateralLeftRange_maximum: " << reference_line_info->rss_info().acc_lat_left_range_maximum();
        ADEBUG << " acc_lateralRightRange: " << reference_line_info->rss_info().acc_lat_right_range_minimum();
        ADEBUG << " acc_lateralRightRange_maximum: " << reference_line_info->rss_info().acc_lat_right_range_maximum();
