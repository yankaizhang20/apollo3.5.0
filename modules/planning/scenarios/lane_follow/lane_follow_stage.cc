...
LaneFollowStage::LaneFollowStage(const ScenarioConfig::StageConfig& config) : Stage(config) {}
...
void LaneFollowStage::RecordDebugInfo(ReferenceLineInfo* reference_line_info, const std::string& name,
...
Status LaneFollowStage::PlanOnReferenceLine(const TrajectoryPoint& planning_start_point, Frame* frame,
        ...
        *heuristic_speed_data = SpeedData(speed_profile);
        ...
        for (auto* optimizer : task_list_) {
                ...
                ADEBUG << "after optimizer " << optimizer->Name() << ":" << reference_line_info->PathSpeedDebugString()
                       << std::endl;
        ...
        if (!(reference_line_info->trajectory_type() == ADCTrajectory::PATH_FALLBACK ||
              reference_line_info->trajectory_type() == ADCTrajectory::SPEED_FALLBACK)) {
...
void LaneFollowStage::GenerateFallbackPathProfile(const ReferenceLineInfo* reference_line_info, PathData* path_data) {
        ...
        for (double s = adc_s; s < max_s; s += unit_s) {
                ...
                common::PathPoint path_point =
                        common::util::MakePathPoint(ref_point.x() + dx, ref_point.y() + dy, 0.0, ref_point.heading(),
...
SLPoint LaneFollowStage::GetStopSL(const ObjectStop& stop_decision, const ReferenceLine& reference_line) const {
        SLPoint sl_point;
        reference_line.XYToSL({stop_decision.stop_point().x(), stop_decision.stop_point().y()}, &sl_point);
        return sl_point;
}
