...
void SidePassScenario::RegisterStages() {
        ...
        s_stage_factory_.Register(
                ...
                [](const ScenarioConfig::StageConfig& config) -> Stage* { return new StageApproachObstacle(config); });
        s_stage_factory_.Register(
                ScenarioConfig::SIDE_PASS_DETECT_SAFETY,
                [](const ScenarioConfig::StageConfig& config) -> Stage* { return new StageDetectSafety(config); });
        s_stage_factory_.Register(
                ScenarioConfig::SIDE_PASS_GENERATE_PATH,
                [](const ScenarioConfig::StageConfig& config) -> Stage* { return new StageGeneratePath(config); });
        s_stage_factory_.Register(
                ScenarioConfig::SIDE_PASS_STOP_ON_WAITPOINT,
                [](const ScenarioConfig::StageConfig& config) -> Stage* { return new StageStopOnWaitPoint(config); });
        s_stage_factory_.Register(
                ScenarioConfig::SIDE_PASS_PASS_OBSTACLE,
                [](const ScenarioConfig::StageConfig& config) -> Stage* { return new StagePassObstacle(config); });
        s_stage_factory_.Register(
                ScenarioConfig::SIDE_PASS_BACKUP,
                [](const ScenarioConfig::StageConfig& config) -> Stage* { return new StageBackup(config); });
}

SidePassScenario::SidePassScenario(const ScenarioConfig& config, const ScenarioContext* scenario_context)
    : Scenario(config, scenario_context) {
        side_pass_context_.scenario_config_.CopyFrom(config.side_pass_config());
}

std::unique_ptr<Stage> SidePassScenario::CreateStage(const ScenarioConfig::StageConfig& stage_config) {
        if (s_stage_factory_.Empty()) {
                RegisterStages();
        }
        auto ptr = s_stage_factory_.CreateObjectOrNull(stage_config.stage_type(), stage_config);
        if (ptr == nullptr) {
                AERROR << "Failed to create stage for config: " << stage_config.DebugString();
                return nullptr;
        }
...
bool SidePassScenario::IsTransferable(const Scenario& current_scenario, const common::TrajectoryPoint& ego_point,
                                      const Frame& frame) {
        // Sanity checks.
        ...
        if (current_scenario.scenario_type() == ScenarioConfig::SIDE_PASS) {
                ...
                const auto front_blocking_obstacle = const_cast<Frame&>(frame).Find(front_blocking_obstacle_id_);
                if (front_blocking_obstacle != nullptr && !front_blocking_obstacle->IsStatic()) {
                ...
                return (current_scenario.GetStatus() != Scenario::ScenarioStatus::STATUS_DONE);
...
bool SidePassScenario::IsSidePassScenario(const Frame& frame) {
        return (IsFarFromDestination(frame) && IsFarFromIntersection(frame) && HasBlockingObstacle(frame));
}
...
bool SidePassScenario::IsFarFromIntersection(const Frame& frame) {
        ...
        const SLBoundary& adc_sl_boundary = frame.reference_line_info().front().AdcSlBoundary();
        const auto& first_encountered_overlaps = frame.reference_line_info().front().FirstEncounteredOverlaps();
        for (const auto& overlap : first_encountered_overlaps) {
                ADEBUG << "AdcSL: " << adc_sl_boundary.ShortDebugString();
                ADEBUG << overlap.first << ", " << overlap.second.DebugString();
                if (overlap.first != ReferenceLineInfo::CROSSWALK && overlap.first != ReferenceLineInfo::SIGNAL &&
...
bool SidePassScenario::HasBlockingObstacle(const Frame& frame) {
        ...
        for (const auto* obstacle : path_decision.obstacles().Items()) {
                ...
                if (IsBlockingObstacleToSidePass(frame, obstacle, side_pass_config.block_obstacle_min_speed(),
                        ...
                        double distance_between_adc_and_obstacle = GetDistanceBetweenADCAndObstacle(frame, obstacle);
                        if (distance_to_closest_blocking_obstacle < 0.0 ||
                            distance_between_adc_and_obstacle < distance_to_closest_blocking_obstacle) {
                                // Only record the closest front blocking obstacle.
                                distance_to_closest_blocking_obstacle = distance_between_adc_and_obstacle;
