...
bool ReferenceLine::Stitch(const ReferenceLine& other) {
        ...
        bool first_join =
                first_sl.s() > 0 && first_sl.s() < other.Length() && std::fabs(first_sl.l()) < kStitchingError;
        ...
        map_path_ = MapPath(
                std::move(std::vector<hdmap::MapPathPoint>(reference_points_.begin(), reference_points_.end())));
        return true;
}
...
common::FrenetFramePoint ReferenceLine::GetFrenetPoint(const common::PathPoint& path_point) const {
        ...
        const double ddl = CartesianFrenetConverter::CalculateSecondOrderLateralDerivative(theta_ref, theta, kappa_ref,
                                                                                           kappa, dkappa_ref, l);
...
ReferencePoint ReferenceLine::GetReferencePoint(const double x, const double y) const {
        ...
        double s = ReferenceLine::FindMinDistancePoint(reference_points_[index_start], s0, reference_points_[index_end],
                                                       s1, x, y);
...
ReferencePoint ReferenceLine::InterpolateWithMatchedIndex(const ReferencePoint& p0, const double s0,
...
ReferencePoint ReferenceLine::Interpolate(const ReferencePoint& p0, const double s0, const ReferencePoint& p1,
        ...
        if (!p0.lane_waypoints().empty() && !p1.lane_waypoints().empty()) {
                ...
                if (p1_waypoint.lane->id().id() != p0_waypoint.lane->id().id() && p1_waypoint.s - (s1 - s) >= 0) {
...
const std::vector<ReferencePoint>& ReferenceLine::reference_points() const { return reference_points_; }
...
bool ReferenceLine::GetApproximateSLBoundary(const common::math::Box2d& box, const double start_s, const double end_s,
                                             SLBoundary* const sl_boundary) const {
        double s = 0.0;
        double l = 0.0;
        double distance = 0.0;
        if (!map_path_.GetProjectionWithHueristicParams(box.center(), start_s, end_s, &s, &l, &distance)) {
...
std::vector<hdmap::LaneSegment> ReferenceLine::GetLaneSegments(const double start_s, const double end_s) const {
        return map_path_.GetLaneSegments(start_s, end_s);
}

bool ReferenceLine::GetSLBoundary(const hdmap::Polygon& polygon, SLBoundary* const sl_boundary) const {
        double start_s(std::numeric_limits<double>::max());
        double end_s(std::numeric_limits<double>::lowest());
        double start_l(std::numeric_limits<double>::max());
        double end_l(std::numeric_limits<double>::lowest());
...
std::string ReferenceLine::DebugString() const {
        ...
        return apollo::common::util::StrCat("point num:", reference_points_.size(),
