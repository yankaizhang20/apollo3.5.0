...
QpSplineReferenceLineSmoother::QpSplineReferenceLineSmoother(const ReferenceLineSmootherConfig& config)
    : ReferenceLineSmoother(config) {
        if (FLAGS_use_osqp_optimizer_for_reference_line) {
                ...
                spline_solver_.reset(new ActiveSetSpline2dSolver(t_knots_, config.qp_spline().spline_order()));
        }
}
...
bool QpSplineReferenceLineSmoother::Smooth(const ReferenceLine& raw_reference_line,
        ...
        ADEBUG << "QpSplineReferenceLineSmoother solve time is " << (end - start) * 1000 << " ms.";
        ...
        for (std::uint32_t i = 0; i < config_.num_of_total_points() && t < end_t; ++i, t += resolution) {
                ...
                const double kappa = CurveMath::ComputeCurvature(spline.DerivativeX(t), spline.SecondDerivativeX(t),
...
bool QpSplineReferenceLineSmoother::Sampling() {
        const double length = anchor_points_.back().path_point.s() - anchor_points_.front().path_point.s();
        uint32_t num_spline =
...
bool QpSplineReferenceLineSmoother::AddConstraint() {
        ...
        const double scale = (anchor_points_.back().path_point.s() - anchor_points_.front().path_point.s()) /
