...
class OpenSpacePlanning : public PlanningBase {
    public:
        OpenSpacePlanning() { planner_dispatcher_ = std::make_unique<StdPlannerDispatcher>(); }
        ...
        void AddPublishedSpeed(planning_internal::Debug* debug, ADCTrajectory* const ptr_trajectory_pb);

        void AddPublishedAcceleration(planning_internal::Debug* debug, ADCTrajectory* const ptr_trajectory_pb);

        void ExportOpenSpaceChart(planning_internal::Debug* debug, ADCTrajectory* const ptr_trajectory_pb);

        void FillPlanningPb(const double timestamp, ADCTrajectory* const ptr_trajectory_pb) override;

    private:
        apollo::common::Status InitFrame(const uint32_t sequence_num,
