...
class NaviPlanning : public PlanningBase {
    public:
        NaviPlanning() { planner_dispatcher_ = std::make_unique<NaviPlannerDispatcher>(); }
    ...
    private:
        common::Status InitFrame(const uint32_t sequence_num, const common::TrajectoryPoint& planning_start_point,
         ...
         *far
         */
        void GetRightNeighborLanesInfo(std::vector<std::pair<std::string, double>>* const lane_info_group);
