...
class Frame {
    public:
        ...
        explicit Frame(uint32_t sequence_num, const LocalView &local_view,
                       const common::TrajectoryPoint &planning_start_point, const double start_time,
                       const common::VehicleState &vehicle_state, ReferenceLineProvider *reference_line_provider,
                       ADCTrajectory *output_trajectory);

        explicit Frame(uint32_t sequence_num, const LocalView &local_view,
                       const common::TrajectoryPoint &planning_start_point, const double start_time,
                       const common::VehicleState &vehicle_state, ADCTrajectory *output_trajectory);

        const common::TrajectoryPoint &PlanningStartPoint() const;

        common::Status Init(const std::list<ReferenceLine> &reference_lines,
        ...
        const Obstacle *CreateStopObstacle(ReferenceLineInfo *const reference_line_info, const std::string &obstacle_id,
                                           const double obstacle_s);

        const Obstacle *CreateStopObstacle(const std::string &obstacle_id, const std::string &lane_id,
                                           const double lane_s);
        ...
        planning_internal::OpenSpaceDebug *mutable_open_space_debug() { return &open_space_debug_; }

        const planning_internal::OpenSpaceDebug &open_space_debug() { return open_space_debug_; }

        std::vector<common::TrajectoryPoint> *mutable_last_stitching_trajectory() { return &stitching_trajectory_; }

        const std::vector<common::TrajectoryPoint> &last_stitching_trajectory() { return stitching_trajectory_; }
    ...
    private:
         ...
         */
        const Obstacle *CreateStaticVirtualObstacle(const std::string &id, const common::math::Box2d &box);
