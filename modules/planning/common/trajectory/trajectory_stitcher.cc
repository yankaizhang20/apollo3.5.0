...
std::vector<TrajectoryPoint> TrajectoryStitcher::ComputeReinitStitchingTrajectory(const VehicleState& vehicle_state) {
...
std::vector<TrajectoryPoint> TrajectoryStitcher::ComputeStitchingTrajectory(
        ...
        double forward_rel_time =
                prev_trajectory->TrajectoryPointAt(static_cast<uint32_t>(time_matched_index)).relative_time() +
                planning_cycle_time;
        ...
        std::vector<TrajectoryPoint> stitching_trajectory(
                prev_trajectory->begin() + std::max(0, static_cast<int>(matched_index - kNumPreCyclePoint)),
