...
FrameHistory::FrameHistory() : IndexedQueue<uint32_t, Frame>(FLAGS_max_history_frame_num) {}
...
Frame::Frame(uint32_t sequence_num, const LocalView &local_view, const common::TrajectoryPoint &planning_start_point,
             const double start_time, const common::VehicleState &vehicle_state,
             ReferenceLineProvider *reference_line_provider, ADCTrajectory *output_trajectory)
...
Frame::Frame(uint32_t sequence_num, const LocalView &local_view, const common::TrajectoryPoint &planning_start_point,
             const double start_time, const common::VehicleState &vehicle_state, ADCTrajectory *output_trajectory)
    : Frame(sequence_num, local_view, planning_start_point, start_time, vehicle_state, nullptr, output_trajectory) {}
...
bool Frame::Rerouting() {
        ...
        auto point = common::util::MakePointENU(vehicle_state_.x(), vehicle_state_.y(), vehicle_state_.z());
        double s = 0.0;
        double l = 0.0;
        ...
        PlanningContext::MutablePlanningStatus()->mutable_rerouting()->set_need_rerouting(true);

        PlanningContext::MutablePlanningStatus()->mutable_rerouting()->mutable_routing_request()->CopyFrom(request);

        monitor_logger_buffer_.INFO("Planning send Rerouting request");
        return true;
}

const std::list<ReferenceLineInfo> &Frame::reference_line_info() const { return reference_line_info_; }
 ...
 */
const Obstacle *Frame::CreateStopObstacle(ReferenceLineInfo *const reference_line_info, const std::string &obstacle_id,
 ...
 *         mainly used for virtual stop wall
 */
const Obstacle *Frame::CreateStopObstacle(const std::string &obstacle_id, const std::string &lane_id,
                                          const double lane_s) {
        if (!hdmap_) {
                AERROR << "Invalid HD Map.";
                return nullptr;
...
const ReferenceLineInfo *Frame::DriveReferenceLineInfo() const { return drive_reference_line_info_; }
