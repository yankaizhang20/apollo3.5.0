...
ReferenceLineInfo::ReferenceLineInfo(const common::VehicleState& vehicle_state,
                                     const TrajectoryPoint& adc_planning_point, const ReferenceLine& reference_line,
...
bool ReferenceLineInfo::Init(const std::vector<const Obstacle*>& obstacles) {
        ...
        //@zyk:找到车辆包围框的sl范围
        if (!reference_line_.GetSLBoundary(vehicle_box, &sl_boundary_info_.vehicle_sl_boundary_)) {
        ...
        //@zyk:找到规划起始点包围框的sl范围
        if (!reference_line_.GetSLBoundary(box, &sl_boundary_info_.adc_sl_boundary_)) {
        ...
        if (sl_boundary_info_.adc_sl_boundary_.end_s() < 0 ||
            sl_boundary_info_.adc_sl_boundary_.start_s() > reference_line_.Length()) {
                AWARN << "Vehicle SL " << sl_boundary_info_.adc_sl_boundary_.ShortDebugString()
        ...
        is_on_reference_line_ = reference_line_.IsOnLane(sl_boundary_info_.adc_sl_boundary_);
...
void ReferenceLineInfo::SetJunctionRightOfWay(double junction_s, bool is_protected) {
        auto* right_of_way = PlanningContext::MutablePlanningStatus()->mutable_right_of_way();
...
bool ReferenceLineInfo::CheckChangeLane() const {
        ...
        for (const auto* obstacle : path_decision_.obstacles().Items()) {
                ...
                if (sl_boundary.start_l() < -kLateralShift || sl_boundary.end_l() > kLateralShift) {
                        continue;
                }
                constexpr double kChangeLaneIgnoreDistance = 50.0;
                if ((obstacle->IsVirtual() || obstacle->IsStatic()) &&
                    sl_boundary.start_s() < sl_boundary_info_.adc_sl_boundary_.end_s() + kChangeLaneIgnoreDistance &&
                ...
                const float kForwardSafeDistance =
                        std::max(kForwardMinSafeDistance,
                                 static_cast<float>((adc_planning_point_.v() - obstacle->speed()) * kSafeTime));
                const float kBackwardSafeDistance =
                        std::max(kBackwardMinSafeDistance,
                                 static_cast<float>((obstacle->speed() - adc_planning_point_.v()) * kSafeTime));
                if (sl_boundary.end_s() > sl_boundary_info_.adc_sl_boundary_.start_s() - kBackwardSafeDistance &&
                    sl_boundary.start_s() < sl_boundary_info_.adc_sl_boundary_.end_s() + kForwardSafeDistance) {
                        return false;
                }
        }
        return true;
...
const SLBoundary& ReferenceLineInfo::AdcSlBoundary() const { return sl_boundary_info_.adc_sl_boundary_; }

const SLBoundary& ReferenceLineInfo::VehicleSlBoundary() const { return sl_boundary_info_.vehicle_sl_boundary_; }
...
void ReferenceLineInfo::SetTrajectory(const DiscretizedTrajectory& trajectory) { discretized_trajectory_ = trajectory; }
...
bool ReferenceLineInfo::IsUnrelaventObstacle(const Obstacle* obstacle) {
        ...
        if (is_on_reference_line_ &&
            obstacle->PerceptionSLBoundary().end_s() < sl_boundary_info_.adc_sl_boundary_.end_s() &&
...
const DiscretizedTrajectory& ReferenceLineInfo::trajectory() const { return discretized_trajectory_; }
...
void ReferenceLineInfo::MakeMainMissionCompleteDecision(DecisionResult* decision_result) const {
        ...
        if (common::util::DistanceXY(adc_pos, main_stop.stop_point()) > FLAGS_destination_check_distance) {
                return;
        }

        auto mission_complete = decision_result->mutable_main_decision()->mutable_mission_complete();
        if (ReachedDestination()) {
                PlanningContext::MutablePlanningStatus()->mutable_destination()->set_has_passed_destination(true);
        } else {
...
void ReferenceLineInfo::ExportEngageAdvice(EngageAdvice* engage_advice) const {
        ...
        auto* prev_advice = PlanningContext::MutablePlanningStatus()->mutable_engage_advice();
        ...
        if (!IsDrivable()) {
                ...
                // check heading
                auto ref_point = reference_line_.GetReferencePoint(sl_boundary_info_.adc_sl_boundary_.end_s());
