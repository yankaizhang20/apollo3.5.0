...
std::vector<SpeedPoint> SpeedProfileGenerator::GenerateInitSpeedProfile(const TrajectoryPoint& planning_init_point,
...
std::vector<SpeedPoint> SpeedProfileGenerator::GenerateSpeedHotStart(const TrajectoryPoint& planning_init_point) {
...
SpeedData SpeedProfileGenerator::GenerateStopProfileFromPolynomial(const double init_speed, const double init_acc) {
        AERROR << "Slowing down the car with polynomial.";
        constexpr double kMaxT = 4.0;
        for (double t = 2.0; t <= kMaxT; t += 0.5) {
                for (double s = 0.0; s < std::min(50.0, EgoInfo::Instance()->front_clear_distance() - 0.3); s += 1.0) {
...
bool SpeedProfileGenerator::IsValidProfile(const QuinticPolynomialCurve1d& curve) {
...
SpeedData SpeedProfileGenerator::GenerateFixedDistanceCreepProfile(const double distance, const double max_speed) {
...
SpeedData SpeedProfileGenerator::GenerateFixedSpeedCreepProfile(const double distance, const double max_speed) {
        constexpr double kProceedingSpeed = 2.23; // (5mph proceeding speed)
        const double proceeding_speed = std::min(max_speed, kProceedingSpeed);
        ...
        for (double s = 0.0; s < distance; s += kDeltaS) {
                speed_data.AppendSpeedPoint(s, s / proceeding_speed, proceeding_speed, 0.0, 0.0);
        }
        return speed_data;
}
