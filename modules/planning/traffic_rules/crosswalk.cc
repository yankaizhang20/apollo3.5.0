...
void Crosswalk::MakeDecisions(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
        ...
        for (int i = 0; i < mutable_crosswalk_status->stop_time_size(); ++i) {
                stop_times.insert({mutable_crosswalk_status->stop_time(i).obstacle_id(),
        ...
        if (firsts_crosswalk_to_stop) {
                ...
                mutable_crosswalk_status->clear_stop_time();
                for (auto it = crosswalk_stop_timer[crosswalk].begin(); it != crosswalk_stop_timer[crosswalk].end();
                     ++it) {
                        ...
                        ADEBUG << "UPDATE stop_time: id[" << crosswalk << "] obstacle_id[" << it->first
...
bool Crosswalk::CheckStopForObstacle(ReferenceLineInfo* const reference_line_info,
        ...
        if (obstacle_type != PerceptionObstacle::PEDESTRIAN && obstacle_type != PerceptionObstacle::BICYCLE &&
            obstacle_type != PerceptionObstacle::UNKNOWN_MOVABLE && obstacle_type != PerceptionObstacle::UNKNOWN) {
        ...
        reference_line.XYToSL({perception_obstacle.position().x(), perception_obstacle.position().y()},
                              &obstacle_sl_point);
        ...
        ADEBUG << "obstacle_id[" << obstacle_id << "] type[" << obstacle_type_name << "] crosswalk_id[" << crosswalk_id
               << "] obstacle_l[" << obstacle_sl_point.l() << "] within_expanded_crosswalk_area["
               << in_expanded_crosswalk << "] obstacle_l_distance[" << obstacle_l_distance << "] on_lane[" << is_on_lane
                ...
                if (is_on_road) {
                        ...
                        if (obstacle_sl_point.s() > adc_end_edge_s) {
                                ...
                                ADEBUG << "need_stop(<=l1): obstacle_id[" << obstacle_id << "] type["
                                       << obstacle_type_name << "] s[" << obstacle_sl_point.s() << "] adc_end_edge_s[ "
                                       << adc_end_edge_s << "] crosswalk_id[" << crosswalk_id << "] ON_ROAD";
                        }
                } else {
                        ...
                        if (is_path_cross) {
                                ...
                                const auto adc_path_point = Vec2d(EgoInfo::Instance()->start_point().path_point().x(),
...
int Crosswalk::BuildStopDecision(Frame* const frame, ReferenceLineInfo* const reference_line_info,
        ...
        auto* obstacle =
                frame->CreateStopObstacle(reference_line_info, virtual_obstacle_id, crosswalk_overlap->start_s);
        if (!obstacle) {
