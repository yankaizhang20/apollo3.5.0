...
bool ChangeLane::CreateGuardObstacle(const ReferenceLineInfo* reference_line_info, Obstacle* obstacle) {
        ...
        const auto& reference_line = reference_line_info->reference_line();
        const double end_s = std::min(reference_line.Length(), reference_line_info->AdcSlBoundary().end_s() +
...
Status ChangeLane::ApplyRule(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
        ...
        if (!overtake_obstacles_.empty()) {
                ...
                for (const auto* obstacle : overtake_obstacles_) {
                        ...
                        path_decision->AddLongitudinalDecision(TrafficRuleConfig::RuleId_Name(Id()), obstacle->Id(),
                                                               overtake);
                }
        }
        return Status::OK();
