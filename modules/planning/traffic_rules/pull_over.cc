...
Status PullOver::ApplyRule(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
        ...
        if (GetPullOverStopPoint(&stop_point) != 0) {
                BuildInLaneStop(stop_point); //@zyk:停在车道上
                ADEBUG << "Could not find a safe pull over point. STOP in-lane";
        } else {
                BuildPullOverStop(stop_point); //@zyk:pull over
        }
...
bool PullOver::IsPullOver() const {
        ...
        return (planning_status->has_pull_over() && planning_status->pull_over().in_pull_over());
}
...
PullOver::ValidateStopPointCode PullOver::IsValidStop(const common::SLPoint& stop_point_sl) const {
        ...
        const auto& pull_over_status = PlanningContext::MutablePlanningStatus()->pull_over();
        ...
        parking_spot_boundary.set_start_s(stop_point_sl.s() - adc_length - PARKING_SPOT_LONGITUDINAL_BUFFER);
        parking_spot_boundary.set_end_s(stop_point_sl.s() + PARKING_SPOT_LONGITUDINAL_BUFFER);
        parking_spot_boundary.set_start_l(stop_point_sl.l() - adc_width / 2 - config_.pull_over().buffer_to_boundary());
...
int PullOver::GetPullOverStopPoint(PointENU* stop_point) {
        ...
        if (pull_over_status.has_start_point() && pull_over_status.has_stop_point()) {
                ...
                if (ret == OK) {
                        ...
                        if (ret == PASS_DEST_POINT_TOO_FAR) {
                                ...
                                //@zyk:max_failure_count=10 ，超过此值将会在车道上停车
                                retry = false;
                        }
                }
        }
        ...
        failure_count_++;
        if (stop_point_.has_x() && stop_point_.has_y() && failure_count_ < config_.pull_over().max_failure_count()) {
...
bool PullOver::OnOverlap(const double start_s, const double end_s) {
        ...
        // crosswalk
        const std::vector<PathOverlap>& crosswalk_overlaps = reference_line.map_path().crosswalk_overlaps();
        ...
        // junction
        const std::vector<PathOverlap>& pnc_junction_overlaps = reference_line.map_path().pnc_junction_overlaps();
        for (const auto& pnc_junction_overlap : pnc_junction_overlaps) {
                if (start_s <= pnc_junction_overlap.end_s && end_s >= pnc_junction_overlap.start_s) {
                        ADEBUG << "s[" << start_s << ", " << end_s << "] on pnc_junction_overlap["
                               << pnc_junction_overlap.object_id << "] s[" << pnc_junction_overlap.start_s << ", "
        ...
        // clear_area
        const std::vector<PathOverlap>& clear_area_overlaps = reference_line.map_path().clear_area_overlaps();
        ...
        // speed_bump
        const std::vector<PathOverlap>& speed_bump_overlaps = reference_line.map_path().speed_bump_overlaps();
...
int PullOver::FindPullOverStop(const double stop_point_s, PointENU* stop_point) {
        ...
        road_right_width = std::min(std::min(parking_spot_end_s_road_right_width, adc_center_s_road_right_width),
                                    parking_spot_start_s_road_right_width);

        common::SLPoint stop_point_sl;
        stop_point_sl.set_s(stop_point_s);
        stop_point_sl.set_l(-(road_right_width - adc_width / 2 - config_.pull_over().buffer_to_boundary()));
...
int PullOver::FindPullOverStop(PointENU* stop_point) {
        ...
        //@zyk:max_check_distance==60
        while (check_s < reference_line.Length() && total_check_length < config_.pull_over().max_check_distance()) {
                ...
                //@zyk：NONE或CITY_DRIVING类型的车道不能pull over
                ...
                ADEBUG << "check_length: " << check_length << "; plan_distance:" << config_.pull_over().plan_distance();
                //@zyk:停车点距离必须大于规划距离
...
bool PullOver::CheckPullOverComplete() {
        ...
        if (adc_end_edge_s > stop_point_sl.s()) {
                PlanningContext::MutablePlanningStatus()->mutable_pull_over()->set_status(PullOverStatus::DONE);
...
bool PullOver::CheckStopDeceleration(const double stop_line_s) const {
        double stop_deceleration = util::GetADCStopDeceleration(reference_line_info_, stop_line_s,
...
int PullOver::BuildPullOverStop(const PointENU& stop_point) {
        ...
        auto* pull_over_status = PlanningContext::MutablePlanningStatus()->mutable_pull_over();

        //@zyk:pull_over预计行驶距离30m
...
int PullOver::BuildInLaneStop(const PointENU& pull_over_stop_point) {
        ...
        if (inlane_stop_point_.has_x() && inlane_stop_point_.has_y()) {
                ...
                if (CheckStopDeceleration(stop_point_sl.s())) {
                        ...
                        ADEBUG << "BuildInLaneStop using previously set inlane_stop_point: s[" << stop_point_sl.s()
                               << "] dist[" << stop_point_sl.s() - adc_front_edge_s << "] POINT:("
        ...
        if (!inlane_stop_point_set) {
                if (pull_over_stop_point.has_x() && pull_over_stop_point.has_y()) {
                        ...
                        if (CheckStopDeceleration(stop_point_sl.s())) {
                                inlane_stop_point_set = true;
                                ADEBUG << "BuildInLaneStop using pull_over_stop_point: s[" << stop_point_sl.s()
        ...
        if (!inlane_stop_point_set) {
                double adc_speed = common::VehicleStateProvider::Instance()->linear_velocity();
                double stop_distance = (adc_speed * adc_speed) / (2 * config_.pull_over().max_stop_deceleration());
        ...
        const auto& inlane_point = reference_line.GetReferencePoint(stop_point_sl.s());
...
int PullOver::BuildStopDecision(const std::string& vistual_obstacle_id_postfix, const double stop_line_s,
                                const PointENU& stop_point, const double stop_point_heading) {
        const auto& reference_line = reference_line_info_->reference_line();
        ...
        //@zyk：构建虚拟停止墙，并将障碍加入到reference_line_info
        const auto& pull_over_reason = PlanningContext::MutablePlanningStatus()->pull_over().reason();
        std::string virtual_obstacle_id =
                PULL_OVER_VO_ID_PREFIX + PullOverStatus_Reason_Name(pull_over_reason) + vistual_obstacle_id_postfix;

        auto* obstacle = frame_->CreateStopObstacle(reference_line_info_, virtual_obstacle_id, stop_line_s);
        if (!obstacle) {
