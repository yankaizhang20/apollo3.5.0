...
SignalLight::SignalLight(const TrafficRuleConfig& config) : TrafficRule(config) {}

Status SignalLight::ApplyRule(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
        //@zyk:检查当前路况下是否有佳通信号灯区域
...
void SignalLight::ReadSignals(const std::shared_ptr<TrafficLightDetection>& traffic_light) {
        detected_signals_.clear();
        if (traffic_light == nullptr) {
                return;
        }
        const double delay = traffic_light->header().timestamp_sec() - Clock::NowInSeconds();
...
void SignalLight::MakeDecisions(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
        ...
        for (auto& signal_light : signal_lights_from_path_) {
                ...
                double stop_deceleration = util::GetADCStopDeceleration(reference_line_info, signal_light.start_s,
                ...
                const double monitor_forward_distance = std::max(kTrafficLightLookForwardDistanceConfGuard,
                ...
                if (signal_light.start_s - adc_front_edge_s > monitor_forward_distance) {
                        ...
                        common::math::Vec2d adc_position = {common::VehicleStateProvider::Instance()->x(),
                                                            common::VehicleStateProvider::Instance()->y()};

                        const double distance = common::util::DistanceXY(signal_light_point, adc_position);
                        ADEBUG << "traffic_light[" << signal_light.object_id << "] start_s[" << signal_light.start_s
                ...
                ADEBUG << "traffic_light[" << signal_light.object_id << "] start_s[" << signal_light.start_s
                       << "] color[" << signal.color() << "] stop_deceleration[" << stop_deceleration << "]";
                //@zyk:红灯或者黄灯车速较低情况
                    ...
                    (signal.color() == TrafficLight::YELLOW &&
                     stop_deceleration < config_.signal_light().max_stop_deacceleration_yellow_light())) {
                        ...
                        if (config_.signal_light().righ_turn_creep().enabled() &&
                            reference_line_info->IsRightTurnPath(forward_buffer)) {//@zyk:在右转道
                ...
                if (has_stop) {
                        ...
                        if (adc_front_edge_s - signal_light.start_s < 5.0 && signal.color() == TrafficLight::YELLOW) {
                                is_protected = true;
                        }
                } else {
                        is_protected = true;
                }
                reference_line_info->SetJunctionRightOfWay(signal_light.start_s, is_protected);
        }
}
...
void SignalLight::SetCreepForwardSignalDecision(ReferenceLineInfo* const reference_line_info,
        ...
        if (EgoInfo::Instance()->start_point().v() > config_.signal_light().righ_turn_creep().speed_limit()) {
        ...
        for (const auto& obstacle : path_decision->obstacles().Items()) {
                ...
                if (reference_line_info->AdcSlBoundary().end_s() + st_boundary.min_s() < stop_s + creep_s_buffer) {
                        AERROR << "Do not creep forward because obstacles are close.";
                        return;
                }
        }
        signal_light->start_s =
                reference_line_info->AdcSlBoundary().end_s() + config_.signal_light().stop_distance() + creep_s_buffer;
...
bool SignalLight::BuildStopDecision(Frame* const frame, ReferenceLineInfo* const reference_line_info,
        ...
        auto* obstacle = frame->CreateStopObstacle(reference_line_info, virtual_obstacle_id, signal_light->start_s);
        if (!obstacle) {
