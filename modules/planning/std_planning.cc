...
Status StdPlanning::Init(const PlanningConfig& config) {
        ...
        //@zyk:FLAGS_traffic_rule_config_filename="/apollo/modules/planning/conf/traffic_rule_config.pb.txt"
...
Status StdPlanning::InitFrame(const uint32_t sequence_num, const TrajectoryPoint& planning_start_point,
                              const double start_time, const VehicleState& vehicle_state,
                              ADCTrajectory* output_trajectory) {
        frame_.reset(new Frame(sequence_num, local_view_, planning_start_point, start_time, vehicle_state,
        ...
        auto status = frame_->Init(reference_lines, segments, reference_line_provider_->FutureRouteWaypoints());
        if (!status.ok()) {
...
Status StdPlanning::Plan(const double current_time_stamp, const std::vector<TrajectoryPoint>& stitching_trajectory,
...
bool StdPlanning::CheckPlanningConfig(const PlanningConfig& config) {
        ...
        if (config.standard_planning_config().planner_public_road_config().scenario_type_size() == 0) {
                return false;
        }
