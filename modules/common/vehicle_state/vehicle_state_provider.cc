...
Status VehicleStateProvider::Update(const localization::LocalizationEstimate &localization,
        ...
        if (chassis.has_speed_mps()) {
                ...
                if (!FLAGS_reverse_heading_vehicle_state && vehicle_state_.gear() == canbus::Chassis::GEAR_REVERSE) {
...
const localization::Pose &VehicleStateProvider::original_pose() const { return original_localization_.pose(); }
...
/*@zyk:主要是在使用VehicleStateProvider::Update更新状态以后，当需要使用vehicle_state_时，经过了一定的时间差t，
外加车辆运动就造成车辆状态的偏差，所以可以使用该函数来对车辆状态进行一次修正，但是修正必须要满足一个条件：当前时间与
update时间(vehicle_state_.timestamp)与之间的时间差必须小于一个阈值，代码中设置为20ms
*/
