...
struct LaneWaypoint {
        ...
        LaneWaypoint(LaneInfoConstPtr lane, const double s, const double l) : lane(CHECK_NOTNULL(lane)), s(s), l(l) {}
...
class MapPathPoint : public common::math::Vec2d {
    public:
        ...
        MapPathPoint(const common::math::Vec2d& point, double heading, std::vector<LaneWaypoint> lane_waypoints)
            : Vec2d(point.x(), point.y()), heading_(heading), lane_waypoints_(std::move(lane_waypoints)) {}
        ...
        void add_lane_waypoint(LaneWaypoint lane_waypoint) { lane_waypoints_.emplace_back(std::move(lane_waypoint)); }
        ...
        static std::vector<MapPathPoint> GetPointsFromLane(LaneInfoConstPtr lane, const double start_s,
                                                           const double end_s);
...
class PathApproximation {
    public:
        ...
        bool GetProjection(const Path& path, const common::math::Vec2d& point, double* accumulate_s, double* lateral,
...
class Path {
    public:
        ...
        bool GetNearestPoint(const common::math::Vec2d& point, double* accumulate_s, double* lateral,
                             double* distance) const;
        bool GetProjectionWithHueristicParams(const common::math::Vec2d& point, const double hueristic_start_s,
        ...
        bool GetProjection(const common::math::Vec2d& point, double* accumulate_s, double* lateral,
                           double* distance) const;
        ...
        const std::vector<LaneSegment>& lane_segments_to_next_point() const { return lane_segments_to_next_point_; }
