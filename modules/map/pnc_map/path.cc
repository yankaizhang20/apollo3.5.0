...
bool FindLaneSegment(const MapPathPoint& p1, const MapPathPoint& p2, LaneSegment* const lane_segment) {
...
std::vector<MapPathPoint> MapPathPoint::GetPointsFromLane(LaneInfoConstPtr lane, const double start_s,
                                                          const double end_s) {
...
std::string PathOverlap::DebugString() const { return common::util::StrCat(object_id, " ", start_s, " ", end_s); }

Path::Path(const std::vector<MapPathPoint>& path_points) : path_points_(path_points) { Init(); }

Path::Path(std::vector<MapPathPoint>&& path_points) : path_points_(std::move(path_points)) { Init(); }

Path::Path(const std::vector<MapPathPoint>& path_points, const std::vector<LaneSegment>& lane_segments)
...
Path::Path(std::vector<MapPathPoint>&& path_points, std::vector<LaneSegment>&& lane_segments)
    : path_points_(std::move(path_points)), lane_segments_(std::move(lane_segments)) {
        Init();
}
...
Path::Path(const std::vector<LaneSegment>& segments) : lane_segments_(segments) {
        for (const auto& segment : lane_segments_) {
                const auto points = MapPathPoint::GetPointsFromLane(segment.lane, segment.start_s, segment.end_s);
...
Path::Path(std::vector<LaneSegment>&& segments) : lane_segments_(std::move(segments)) {
        for (const auto& segment : lane_segments_) {
                const auto points = MapPathPoint::GetPointsFromLane(segment.lane, segment.start_s, segment.end_s);
                path_points_.insert(path_points_.end(), points.begin(), points.end());
        }
...
void Path::GetAllOverlaps(GetOverlapFromLaneFunc GetOverlaps_from_lane,
        ...
        std::sort(overlaps->begin(), overlaps->end(), [](const PathOverlap& overlap1, const PathOverlap& overlap2) {
                ...
                if (index.id < num_segments_ && !ref_point.lane_waypoints().empty()) {
                        ...
                        if (lane_segment.lane != nullptr) {
                                ...
                                point.add_lane_waypoint(LaneWaypoint(
                                        lane_segment.lane, lane_segment.start_s + index.offset, ref_lane_waypoint.l));
                        }
                }
...
InterpolatedIndex Path::GetIndexFromS(double s) const {
        ...
        int high = (next_sample_id < num_sample_points_ ? std::min(num_points_, last_point_index_[next_sample_id] + 1)
                                                        : num_points_);
        while (low + 1 < high) {
...
std::vector<hdmap::LaneSegment> Path::GetLaneSegments(const double start_s, const double end_s) const {
...
bool Path::GetProjectionWithHueristicParams(const Vec2d& point, const double hueristic_start_s,
                                            const double hueristic_end_s, double* accumulate_s, double* lateral,
        ...
        *min_distance = std::numeric_limits<double>::infinity();
        ...
        int end_interpolation_index = static_cast<int>(std::fmin(num_segments_, GetIndexFromS(hueristic_end_s).id + 1));
...
void PathApproximation::InitDilute(const Path& path) {
        ...
        for (int i = 0; i < num_points_ - 1; ++i) {
                max_error_per_segment_.push_back(compute_max_error(path, original_ids_[i], original_ids_[i + 1]));
        }
}
...
void PathApproximation::InitProjections(const Path& path) {
        ...
        for (int i = 0; i + 1 < num_original_points; ++i) {
                CHECK_LE(max_original_projections_to_left_[i], max_original_projections_to_left_[i + 1] + kMathEpsilon);
        }
...
bool PathApproximation::GetProjection(const Path& path, const common::math::Vec2d& point, double* accumulate_s,
        ...
        double min_distance_sqr_with_error =
                Sqr(sqrt(min_distance_sqr) + max_error_per_segment_[estimate_nearest_segment_idx] + max_error_);
        *min_distance = std::numeric_limits<double>::infinity();
