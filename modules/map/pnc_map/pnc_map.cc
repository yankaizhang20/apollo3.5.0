...
DEFINE_double(look_backward_distance, 30, "look backward this distance when creating reference line from routing");
...
void PncMap::UpdateNextRoutingWaypointIndex(int cur_index) {
        ...
        while (next_routing_waypoint_index_ != 0 && next_routing_waypoint_index_ < routing_waypoint_index_.size() &&
...
std::vector<routing::LaneWaypoint> PncMap::FutureRouteWaypoints() const {
        ...
        return std::vector<routing::LaneWaypoint>(waypoints.begin() + next_routing_waypoint_index_, waypoints.end());
}
...
bool PncMap::IsNewRouting(const routing::RoutingResponse &routing) const { return IsNewRouting(routing_, routing); }

bool PncMap::IsNewRouting(const routing::RoutingResponse &prev, const routing::RoutingResponse &routing) {
        ...
        for (int road_index = 0; road_index < routing.road_size(); ++road_index) {
                ...
                for (int passage_index = 0; passage_index < road_segment.passage_size(); ++passage_index) {
                        ...
                        for (int lane_index = 0; lane_index < passage.segment_size(); ++lane_index) {
                                ...
                                //@zyk:route_indices_存放所有的LaneSegment,及所属的roadsegment和passage标号
...
bool PncMap::GetRouteSegments(const VehicleState &vehicle_state, const double backward_length,
        ...
        //@zyk:找到车辆当前所属车道和最近车道等，得到adc_waypoint_的车道id和累积距离s
...
bool PncMap::ExtendSegments(const RouteSegments &segments, double start_s, double end_s,
        ...
        if (start_s < 0) {
                ...
                while (extend_s > kRouteEpsilon) {
                        if (s <= kRouteEpsilon) {
                                ...
                                if (lane == nullptr || unique_lanes.find(lane->id().id()) != unique_lanes.end()) {
                                        break;
                                }
                                s = lane->total_length();
                        } else {
...
void PncMap::AppendLaneToPoints(LaneInfoConstPtr lane, const double start_s, const double end_s,
